...
@available(iOS 14, macOS 11, watchOS 7, tvOS 14, *)
public extension Binding where Value == SwiftBrickWrapper<Any>.NavigationPath {
    /// Any changes can be made to the screens array passed to the transform closure. If those
    /// changes are not supported within a single update by SwiftUI, the changes will be
    /// applied in stages.
    @_disfavoredOverload
    func withDelaysIfUnsupported(_ transform: (inout SwiftBrickWrapper<Any>.NavigationPath) -> Void, onCompletion: (() -> Void)? = nil) {
        let start = wrappedValue
        let end = apply(transform, to: start)

...
public extension ObservableObject {
    ...
    @_disfavoredOverload
    @MainActor
    func withDelaysIfUnsupported(_ keyPath: WritableKeyPath<Self, SwiftBrickWrapper<Any>.NavigationPath>, transform: (inout SwiftBrickWrapper<Any>.NavigationPath) -> Void, onCompletion: (() -> Void)? = nil) {
        let start = self[keyPath: keyPath]
        let end = apply(transform, to: start)
    ...
    /// applied in stages.
    @MainActor
    func withDelaysIfUnsupported(_ keyPath: WritableKeyPath<Self, SwiftBrickWrapper<Any>.NavigationPath>, transform: (inout SwiftBrickWrapper<Any>.NavigationPath) -> Void) async {
        let start = self[keyPath: keyPath]
        let end = apply(transform, to: start)

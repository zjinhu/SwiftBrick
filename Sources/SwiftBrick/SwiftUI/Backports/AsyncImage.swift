...
public extension SwiftBrickWrapper where Wrapped == Any {
    ...
    // An iOS 13+ async/await SwiftBrickWrapper implementation
    private struct _AsyncImage<Content: View>: View {
        ...
        var content: (SwiftBrickWrapper<Any>.AsyncImagePhase) -> Content
        ...
        init(url: URL?, scale: CGFloat = 1, transaction: Transaction = Transaction(), @ViewBuilder content: @escaping (SwiftBrickWrapper<Any>.AsyncImagePhase) -> Content) {
            self.url = url
            self.scale = scale
            self.transaction = transaction

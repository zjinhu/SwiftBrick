...
extension SwiftBrickWrapper<Any> {
    ...
    public struct PresentationBackgroundInteraction: Hashable {
        enum Interaction: Hashable {
            case automatic
            case enabled
            case upThrough(detent: SwiftBrickWrapper.PresentationDetent)
            case disabled
        }
        ...
        public static func enabled(upThrough detent: SwiftBrickWrapper.PresentationDetent) -> Self { .init(interaction: .upThrough(detent: detent))}
...
public extension SwiftBrickWrapper where Wrapped: View {
    ...
    @ViewBuilder
    func presentationBackgroundInteraction(_ interaction: SwiftBrickWrapper<Any>.PresentationBackgroundInteraction) -> some View {
        #if os(iOS)
        if #available(iOS 15, *) {
...
private extension SwiftBrickWrapper where Wrapped == Any {
    struct Representable: UIViewControllerRepresentable {
        let interaction: SwiftBrickWrapper<Any>.PresentationBackgroundInteraction
...
private extension SwiftBrickWrapper.Representable {
    final class Controller: UIViewController, UISheetPresentationControllerDelegate {
        var interaction: SwiftBrickWrapper<Any>.PresentationBackgroundInteraction
        weak var _delegate: UISheetPresentationControllerDelegate?

        init(interaction: SwiftBrickWrapper<Any>.PresentationBackgroundInteraction) {
        ...
        func update(interaction: SwiftBrickWrapper<Any>.PresentationBackgroundInteraction) {
            self.interaction = interaction
            ...
            if let controller = parent?.sheetPresentationController {
                controller.animateChanges {
                    ...
                    case .upThrough(let detent):
                        ...
                        let selected = SwiftBrickWrapper<Any>.PresentationDetent(id: .init(rawValue: selectedId.rawValue))
